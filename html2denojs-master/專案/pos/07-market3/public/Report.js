...
Report.html = `
  ...
  <div id="report" class="panel">
    ...
    <div class="center">
      ...
      <br/><br/>
      <button onclick="Report.clear()">刪除</button>
      <button onclick="Report.upload()">確認購買</button>
...
Report.showReport = async function () {
  ...
  Ui.id('total').innerHTML = Report.total + ''
}

Report.showDetail = async function (key) {
  Ui.showPanel('detail')
...
Report.orderListHtml = async function () {
  ...
  for (var order of orders){
    rows.push(Report.orderRowHtml(++i, order))
  }
  Report.total = total
  return rows.join('\n')
}
...
Report.orderRowHtml = function (i, order) {
  ...
  return `<tr><td><input type="checkbox"/></td><td><a onclick="Report.showDetail('Order.${order.guid}')">0${i}</a></td>
              <td>${order.shop}</td>
...
Report.saveOrder = async function (Order) {
