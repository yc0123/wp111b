...
function msgToHtml(msg) {
  ...
  if (msg.comments != null) {
    for (let comment of msg.comments) {
      comments.push(`<li>
        <em class="user"><a href="#msgBy/${comment.user}">${comment.user}</a></em> : 
        ${comment.msg}</li>`)
    }
  }
  console.log('msg=', msg)
  return `<li onclick="Ui.goto('#msgGet/${msg.id}')">
    <em class="user"><a href="#msgBy/${msg.ufrom}">${msg.ufrom}</a></em> 
    <em class="time">${timeFormat(msg.time)}</em> <br/> 
    ${msgFormat(msg.msg)}\n<ul>\n
    ${comments.join('\n')}\n</ul>\n
    </li>`
}
