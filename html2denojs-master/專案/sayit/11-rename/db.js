...
export async function open() {
  ...
  db.query(`CREATE VIEW IF NOT EXISTS vFollowMsg
    ...
    FROM
      follows
    INNER JOIN msgs ON follows.follower=msgs.ufrom
  `)
}
...
export async function msgAdd(msg) {
  db.query(`INSERT INTO msgs (msg, ufrom, uto, comments, time) 
                      VALUES (?,   ?,     ?,   ?,        ?)`, 
                      [msg.msg, msg.ufrom, msg.uto, msg.comments, Date.now()])
}

export async function msgBy(uto) {
  let q = db.query(`SELECT id, msg, ufrom, uto, comments, time
            FROM msgs WHERE uto=?`, [uto])
  let msgs = []
...
export async function msgFollow(follower) {
  let q = db.query(`SELECT follower, msg, id, ufrom, uto, comments, time FROM vFollowMsg WHERE follower=?`, [follower])
  let msgs = []
