...
function submitQuestion() {
    ...
    if (location.protocol != 'file:')
        chat(questionNode.value).then((response)=>{
            ...
            document.querySelector(`#status_${chatId}`).innerHTML = `<a href="#view_${chatId}">檢視</a>`
        }).catch((error)=>{
    ...
    let statusCell = row.insertCell(2); statusCell.innerHTML = `<label id="status_${chatId}">等待中</label>`
