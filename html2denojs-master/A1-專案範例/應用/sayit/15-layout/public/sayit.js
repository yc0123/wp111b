...
function msgToHtml(msg) {
  ...
  if (msg.comments != null) {
    for (let comment of msg.comments) {
      comments.push(`
      <div>
        <p><em class="user"><a href="#msgBy/${comment.user}">${comment.user}</a></em> : 
        ${comment.msg}</p>
      </div>`)
    }
  }
  console.log('msg=', msg)
  return `
    <div class="title" onclick="Ui.goto('#msgGet/${msg.id}')">
      <p>
...
async function msgGet(id) {
  ...
  msg = r.obj
  Ui.show(`<div style="float:right"><button>回應本文</botton></div>${msgToHtml(msg)}`)
}
