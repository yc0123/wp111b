...
export async function open() {
  ...
  db.query(`CREATE INDEX iUsers ON users (id, user);`)
  db.query(`CREATE INDEX iFollows ON follows (id, follower, user);`)
  db.query(`CREATE INDEX iSays ON says (id, ufrom, uto, time);`)
  db.query(`CREATE VIEW vFollowSay
    AS 
    SELECT
      follower,
      user,
    ...
    FROM
      follows
    INNER JOIN says ON follows.follower=says.ufrom
  `)
}
...
export async function sayFollow(follower) {
  let q = db.query(`SELECT follower, msg, id, ufrom, uto, comments, time FROM vFollowSay WHERE follower=?`, [follower])
  let says = []
  for (let [follower, msg, id, ufrom, uto, comments, time] of q) {
    says.push({follower, msg, id, ufrom, uto, comments:JSON.parse(comments), time})
  }
  return says
}
