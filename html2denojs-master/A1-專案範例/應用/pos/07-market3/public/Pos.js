...
<div>
  ...
  <br/><br/>
  <button id="goShop" onclick="Ui.goto('#shop')">回商店</button>
...
Pos.start = async function () {
  if (await Cache.get('user') == null) {
    alert('您尚未登入，無法下訂單，請先登入！')
    Ui.goto('#login')
  }
  Shop.init()
  Ui.show(Pos.html)
  Ui.id('items').innerHTML = Pos.optionList(Shop.setting.items)
  Ui.id('addons').innerHTML = Pos.optionList(Shop.setting.addons)
...
Pos.submit = async function () {
  ...
  // await Shop.incCount()
  Order.time = Date.now()
  Order.user = await Cache.get('user')
  Order.shop = Shop.setting.shop
  Order.submitted = true
  Order.guid = Lib.guid()
  await Cart.saveOrder(Order)
