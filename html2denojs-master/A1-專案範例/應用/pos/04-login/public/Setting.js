...
Setting.save = async function () {
  ...
  await DB.set('Shop.name', Shop.name)
  await DB.set('Shop.address', Shop.address)
  await DB.set('Shop.tel', Shop.tel)
  await DB.set('Shop.items', Ui.id('items').value)
  await DB.set('Shop.addons', Ui.id('addons').value)
